...
class Ship(pygame.sprite.Sprite):
    ...
    def draw(self):
        ...
        if self.character_type.startswith("enemy") and self.is_frozen:
            ...
            # 基于透明度生成 mask（只绘制非透明区域）
            mask = pygame.mask.from_surface(img)

            # 遍历 mask 中的有效像素区域并上色（这能确保冰层只在敌人身体上）
            for x in range(mask.get_size()[0]):
                for y in range(mask.get_size()[1]):
                    if mask.get_at((x, y)):  # 只有非透明像素才涂蓝
                        freeze_overlay.set_at((x, y), (120, 200, 255, 80))  # 半透明冰蓝

            # 冰层叠加（带轻微亮度提升）
            img.blit(freeze_overlay, (0, 0), special_flags=pygame.BLEND_RGBA_ADD)

            # 可选：为冻结状态加微光边缘
            outline = mask.outline()
            for point in outline:
                pygame.draw.circle(img, (180, 240, 255, 120), point, 1)
